...
module.exports = {
  checkSignUp: async (input) => {
    ...
    if (badRequest || !input.gender) return '所有欄位皆為必填'
  ...
  checkProduct: input => {
    const attrs = [
      'name', 'slogan', 'description', 'CategoryId',
      'price', 'inventory', 'spec',
      'releaseDate', 'deadline', 'saleDate', 'status',
      'SeriesId', 'copyright', 'maker'
    ]
    const keys = Object.keys(input)
    const attr = attrs.find(val => !keys.includes(val))
    if (attr) return `" * "資料為必填，缺少 ${attr}。`
  }
}